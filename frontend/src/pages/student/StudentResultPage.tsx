...
export const StudentResultPage: React.FC = () => {
        ...
        try {
          ...
          if (updatedJob.status === 'COMPLETED' || updatedJob.status === 'FAILED') {
            ...
            if (updatedJob.status === 'COMPLETED' && updatedJob.download_url) {
              ...
              setReportError(updatedJob.error_message || "Failed to generate report.");
            }
          }
        } catch {
          clearInterval(pollInterval);
          setGeneratingReport(false);
          setReportError("Failed to check report generation status.");
        }
      }, 1500);
    } catch (err: any) {
      setGeneratingReport(false);
      setReportError(err?.response?.data?.message || "Failed to request report generation.");
    }
  };
  ...
  if (error || !result) {
    const isUnreleased =
      ...
      (error || '').toLowerCase().includes('after the assessment deadline') ||
      (error || '').toLowerCase().includes('not yet available') ||
      (error || '').toLowerCase().includes('evaluation is still in progress');
    ...
    <div className="max-w-5xl mx-auto px-4 py-8">
      ...
      <div className="relative overflow-hidden rounded-3xl bg-white border border-slate-200 p-8 mb-8 shadow-sm">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-4 mt-8 pt-6 border-t border-slate-100">
          ...
          <div className="bg-amber-50/70 rounded-xl p-3.5 border border-amber-200 col-span-2 sm:col-span-1">
            <div className="text-xs text-amber-800 flex items-center gap-1.5 font-medium">
              <span>🪙</span> Coins Awarded
            </div>
            <div className="text-lg font-bold text-amber-900 mt-0.5 font-mono">
              +{result.coins_earned !== undefined ? result.coins_earned : result.correct_questions * 3}
            </div>
          </div>
        </div>
      </div>
      ...
      {reportError && (
        <div className="mb-6 p-4 bg-rose-50 border border-rose-200 text-rose-800 rounded-2xl text-xs font-medium flex items-center justify-between shadow-xs">
          <span>{reportError}</span>
          <button onClick={() => setReportError(null)} className="text-rose-600 hover:text-rose-800 font-bold ml-4">
            Dismiss
          </button>
        </div>
      )}
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {isCorrect && (
                        <>
                          ...
                          <span className="text-xs font-bold px-2 py-0.5 rounded-md bg-amber-50 text-amber-800 border border-amber-200 flex items-center gap-1 font-mono">
                            🪙 +3 Coins
                          </span>
                        </>
                      )}
