...
export const AdminAssessmentAttendancePage: React.FC = () => {
  ...
  }, [assessmentId, currentPage, pageSize, attendanceStatus, attemptStatus, searchQuery]);
  ...
  const handleExport = async (format: 'xlsx' | 'pdf') => {
    ...
    try {
      ...
      setErrorMessage(err.error?.message || err.message || `Failed to export attendance ${format.toUpperCase()}.`);
    } finally {
      ...
      case 'DISQUALIFIED':
        return (
          <Badge variant="danger" size="sm" className="font-semibold flex items-center gap-1">
            <ShieldAlert className="w-3 h-3 text-rose-600" /> Disqualified
          </Badge>
        );
      case 'NOT_STARTED':
        return (
      ...
      default:
        return (
          <Badge variant="warning" size="sm" className="font-semibold flex items-center gap-1">
            <AlertCircle className="w-3 h-3 text-amber-600" /> Absent
          </Badge>
        );
    }
  };
    ...
    <div className="w-full px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            title="Download printable PDF attendance report"
          >
        ...
        <div className="p-4 rounded-xl bg-amber-50 border border-amber-200 flex items-start gap-3 text-amber-900 text-xs">
          ...
          <div className="space-y-0.5">
            ...
            <p className="text-amber-800">
              This assessment is scheduled to start on {new Date(assessment!.start_datetime).toLocaleString()}. Candidates who have not yet started are awaiting the scheduled start time and are not considered absent.
            </p>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-6 gap-3">
        <Card className="p-4 space-y-1">
          <span className="text-[10px] text-slate-500 font-semibold uppercase tracking-wider">Total Candidates</span>
          <div className="text-2xl font-extrabold text-slate-900 font-mono">
            {summary ? summary.total_assigned : '-'}
          </div>
          <span className="text-[10px] text-slate-400">Enrolled candidates</span>
        </Card>
        ...
        <Card className="p-4 space-y-1 bg-amber-50/50 border-amber-200">
          <span className="text-[10px] text-amber-800 font-semibold uppercase tracking-wider">
            {summary?.is_pre_exam ? 'Awaiting Start' : 'Absent'}
          </span>
        ...
        <Card className="p-4 space-y-1 bg-rose-50/50 border-rose-200">
          <span className="text-[10px] text-rose-800 font-semibold uppercase tracking-wider">Disqualified</span>
          <div className="text-2xl font-extrabold text-rose-700 font-mono">
            {summary ? (summary.total_disqualified ?? 0) : '-'}
          </div>
          <span className="text-[10px] text-rose-600">Security violations</span>
        </Card>
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div>
            ...
            >
              ...
              <option value="ATTENDED">Attended (Active / Submitted)</option>
              <option value="NOT_ATTENDED">Absent (Not Attended)</option>
              <option value="NOT_STARTED">Not Started (Awaiting)</option>
              <option value="DISQUALIFIED">Disqualified (Security Violations)</option>
            </select>
          </div>
      ...
      {/* Candidate Attendance Roster Table */}
      <Card className="overflow-hidden p-0">
        {isLoading ? (
          ...
          <div className="py-16 text-center space-y-3">
            ...
            <p className="text-slate-600 text-sm font-semibold">No candidates matching the selected criteria.</p>
          </div>
        ) : (
            ...
            <table className="w-full text-left text-xs font-mono">
              <thead className="bg-slate-50 text-slate-600 border-b border-slate-200 uppercase tracking-wider font-semibold">
                <tr>
                  ...
                  <th className="px-4 py-3.5">EUID</th>
        ...
        {/* Pagination Footer */}
        {totalPages > 1 && (
          <div className="p-4 border-t border-slate-200 flex flex-wrap items-center justify-between gap-4 bg-slate-50/50">
            <div className="text-xs text-slate-500 font-mono">
              Showing {rows.length} of {totalCount} total candidates (Page {currentPage} of {totalPages})
            </div>
            <div className="flex items-center gap-2">
              <Button
                variant="outline"
