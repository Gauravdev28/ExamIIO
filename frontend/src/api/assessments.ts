...
export const terminateAttempt = async (
  attemptId: string,
  reason: string = 'EXAMINATION ABANDONED — BROWSER NAVIGATION / ROOM EXIT'
): Promise<APIResponse<{ attempt_id: string; status: string; is_disqualified: boolean; disqualification_reason: string }>> => {
  const res = await apiClient.post<APIResponse<any>>(`/student/attempts/${attemptId}/terminate/`, { reason });
  return res.data;
};
