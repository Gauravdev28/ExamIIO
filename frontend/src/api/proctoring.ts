...
export const sendProctoringHeartbeat = async (
  attemptId: string
): Promise<{
  status: string;
...
export const reportBrowserEvent = async (
  ...
  if (clientIncidentId) {
    payload.client_incident_id = clientIncidentId;
    payload.client_event_id = clientIncidentId;
  }
  const response = await apiClient.post(`/student/attempts/${attemptId}/proctoring/events/`, payload);
  return response.data;
};
