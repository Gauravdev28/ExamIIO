...
export const ForcePasswordChangeModal: React.FC = () => {
  ...
  const isNameMissing = !user?.display_name || user.display_name === 'Student' || user.display_name.trim() === '' || user.display_name.includes('@');
  const isStudent = user?.role === 'STUDENT';
  const [certificateName, setCertificateName] = useState(
    !isNameMissing && user?.display_name ? user.display_name : ''
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isStudent && !certificateName.trim()) {
      setErrorMessage('Please enter your full name for certificate printing.');
      return;
    }

    if (!isStudent && isNameMissing && (!firstName.trim() || !lastName.trim())) {
    ...
    try {
      await changeUserPassword({
        ...
        first_name: !isStudent ? firstName.trim() || undefined : undefined,
        last_name: !isStudent ? lastName.trim() || undefined : undefined,
        certificate_name: isStudent ? certificateName.trim() : undefined,
      });
      await refreshUser();
    } catch (err: any) {
      const msg =
        err.error?.details?.certificate_name?.[0] ||
          ...
          {isStudent ? (
            <div className="space-y-1 pb-1">
              <label className="block text-xs font-semibold text-slate-700">
                Full Name for Certificate Printing <span className="text-rose-500">*</span>
              </label>
              <input
                type="text"
                required
                ...
                placeholder="e.g. Gaurav Agarwal"
                className="w-full px-3 py-2 rounded-lg bg-white border border-slate-300 text-slate-900 placeholder:text-slate-400 text-xs focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 font-medium"
              />
              <p className="text-[11px] text-slate-500">
                Enter your official full name exactly as it should appear on your Certificate of Participation.
              </p>
            </div>
          ) : isNameMissing ? (
