...
@pytest.mark.django_db
def test_roll_number_can_be_used_for_login(api_client, student_one):
    """Craft Society Phase 1: Roll Number is accepted as a dual login identifier."""
    user, profile = student_one
    url = reverse('accounts:login')
    
    # Attempt login using raw roll number
    response = api_client.post(url, {
