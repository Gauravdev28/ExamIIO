...
class TestProctoringIntegration:
    ...
    def test_student_ingest_browser_telemetry_and_receive_warning(self):
        ...
        assert response.data['disqualified'] is True
        assert response.data['attempt_status'] == AttemptStatus.CANCELLED
        self.attempt.refresh_from_db()
        assert self.attempt.status == AttemptStatus.CANCELLED
        assert self.attempt.is_disqualified is True

        # Test warning acknowledgement API
        session = ProctoringSessionService.get_or_create_session(self.attempt)
        warning = ProctoringWarning.objects.create(
            session=session,
            warning_type='SYSTEM',
            message='System advisory warning'
        )
        assert warning.acknowledged_at is None

        # Acknowledge Warning
        ack_url = f"/api/v1/student/attempts/{self.attempt.id}/proctoring/warnings/{warning.id}/ack/"
