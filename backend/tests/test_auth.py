...
class TestAuthenticationContractAndRegressions:
    ...
    def test_student_login_by_email_and_euid(self, api_client, db):
        ...
        # 3. Roll Number directly also succeeds (Craft Society dual login)
        res_roll = api_client.post(login_url, {
            "identifier": "CS2026AUTH",
            "password": "ValidPass2026!"
        })
        assert res_roll.status_code == 200
        assert res_roll.json()['data']['user']['role'] == "STUDENT"
    ...
    def test_password_hasher_native_detection_and_immutability(self, db):
        from django.contrib.auth.hashers import identify_hasher

        # 1. User created with raw password containing special dollar characters
        user = User.objects.create_user(
            email="dollarpass@institution.edu",
            password="P@$$w0rd!Complex",
            role=Role.STUDENT
        )
        assert user.check_password("P@$$w0rd!Complex") is True
        initial_hash = user.password
        hasher = identify_hasher(initial_hash)
        assert hasher is not None

        # 2. Normal profile update must NEVER mutate password hash
        ...
        # 3. Explicit set_password must update hash
        user.set_password("NewP@$$w0rd!456")
        user.save()
        user.refresh_from_db()
        assert user.password != initial_hash
        assert user.check_password("NewP@$$w0rd!456") is True
