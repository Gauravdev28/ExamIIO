...
class TestAssessmentAudienceTargeting:
    ...
    def test_non_student_rejected_and_all_students_eligible_in_audience(self, draft_assessment, admin_user, cohort_students):
        """Admin accounts cannot be added to target_students; inactive students in Craft Society can be targeted."""
        ...
        # Inactive student in Craft Society is targetable
        s1 = cohort_students["s1"]
        s1.is_active = False
        s1.save()
        ...
        assert str(s1.id) in res["eligible_student_ids"]
...
class TestAssessmentAudienceAndAssignmentBugs:
    ...
    def test_20_non_student_cannot_be_assigned_and_all_students_assignable(self, admin_client, draft_assessment):
        """TEST 20: Non-student accounts cannot be assigned; all students in Craft Society can be assigned."""
        # Non-student account rejected
        url = reverse('assessments:admin-assessment-assignment-list', kwargs={'pk': draft_assessment.id})
        res = admin_client.post(url, {"student_ids": ["invalid-not-a-student"]}, format='json')
        assert res.status_code == status.HTTP_400_BAD_REQUEST

        # Inactive student in Craft Society can be assigned
        inactive_s, _ = StudentService.create_student("inactive_s@univ.edu", "INACT99")
        inactive_s.is_active = False
        inactive_s.save()
        res2 = admin_client.post(url, {"student_ids": [str(inactive_s.id)]}, format='json')
        assert res2.status_code == status.HTTP_201_CREATED
        assert AssessmentAssignment.objects.filter(assessment=draft_assessment, student=inactive_s).count() == 1
