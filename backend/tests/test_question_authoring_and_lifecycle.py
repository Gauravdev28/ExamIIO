...
class TestQuestionAuthoringAndLifecycle:
    ...
    def test_get_or_create_draft_version_branches_new_version(self, admin_user, coding_question):
        """Published questions are immutable: get_or_create_draft_version raises ValidationError; duplicate creates a new question."""
        from rest_framework.exceptions import ValidationError as DRFValidationError
        q, v1 = coding_question
        ...
        with pytest.raises(DRFValidationError) as exc:
            QuestionService.get_or_create_draft_version(q, admin_user)
        assert "immutable" in str(exc.value).lower()

        # Duplication creates a new question in DRAFT with full deep copy
        new_q, new_v = QuestionService.duplicate_question(q, admin_user)
        assert new_q.id != q.id
        assert new_v.status == VersionStatus.DRAFT
        assert new_v.version_number == 1
        assert "Copy" in new_v.title
        assert new_v.coding_config.problem_statement == v1.coding_config.problem_statement
        assert new_v.coding_config.test_cases.count() == v1.coding_config.test_cases.count()
    ...
    def test_assessment_snapshots_remain_locked_across_question_branches(self, admin_user, student_user, coding_question):
        ...
        Published questions cannot be branched; duplicating a question creates a new question and leaves the snapshot intact.
        """
        q, v1 = coding_question
        QuestionService.publish_version(v1, admin_user)
        ...
        # Duplicating q creates q2 in DRAFT
        q2, v2 = QuestionService.duplicate_question(q, admin_user)
        QuestionService.update_draft_version(v2, title="Updated Title for Q2")
        QuestionService.publish_version(v2, admin_user)

        # Verify that snapshot still strictly references original v1
    ...
    def test_referenced_question_deletion_is_blocked(self, api_client, admin_user, coding_question):
        ...
        assert "cannot delete this question because it is used in 1 assessment" in err['message'].lower()
