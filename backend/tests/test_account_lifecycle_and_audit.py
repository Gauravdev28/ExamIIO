...
class TestPrimaryAdminAuthority:
    ...
    def test_primary_admin_api_deactivate_blocked(self, primary_admin, secondary_admin):
        # Secondary admin attempting deactivation receives 403 Forbidden
        client_sec = APIClient()
        client_sec.force_authenticate(user=secondary_admin)
        url = reverse('accounts:admin-administrator-status', kwargs={'pk': primary_admin.id})
        response = client_sec.post(url, {'is_active': False, 'reason': 'Testing deactivation'})
        assert response.status_code == status.HTTP_403_FORBIDDEN

        # Primary admin attempting self deactivation receives 400 Bad Request
        client_prim = APIClient()
        client_prim.force_authenticate(user=primary_admin)
        response_prim = client_prim.post(url, {'is_active': False, 'reason': 'Self deactivation'})
        assert response_prim.status_code == status.HTTP_400_BAD_REQUEST
        assert "PRIMARY_ADMIN_IMMUTABLE" in str(response_prim.data) or "cannot deactivate your own" in str(response_prim.data).lower()
