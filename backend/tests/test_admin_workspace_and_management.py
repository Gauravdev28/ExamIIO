...
class TestAdminDashboardOverview:
    def test_admin_overview_metrics_success(self, api_client, admin_user):
        ...
        StudentProfile.objects.create(
            user=student,
            roll_number="STUDENT-TEST-001",
            euid="CG-STUDENT-TEST-001"
        )
...
class TestAdministratorManagement:
    ...
    def test_toggle_administrator_status(self, api_client):
        primary = User.objects.filter(role=Role.ADMIN, primary_admin_marker='PRIMARY').first()
        if not primary:
            primary = User.objects.create_user(
                email="primary.workspace.test@codeguard.local",
        ...
        api_client.force_authenticate(user=primary)
    ...
    def test_self_deactivation_rejected(self, api_client):
        primary = User.objects.filter(role=Role.ADMIN, primary_admin_marker='PRIMARY').first()
        if not primary:
            primary = User.objects.create_user(
                email="primary.self.deact@codeguard.local",
                password="SecureAdminPass123!",
                role=Role.ADMIN,
                is_staff=True,
                admin_id="EUAD-GAURAV-099",
    ...
    def test_secondary_admin_deactivation_forbidden(self, api_client, admin_user):
        api_client.force_authenticate(user=admin_user)
        target = User.objects.create_user(
            email="target.sec.forbidden@test.edu",
            password="Password@123",
            role=Role.ADMIN,
            is_active=True
        )
