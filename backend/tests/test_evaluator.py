...
class TestEvaluatorIntegration:
    ...
    def mock_code_execution(self, monkeypatch):
        from apps.evaluator.execution import CodeExecutionService, ExecutionResult, ExecutionStatus

        def _mock_execute(request, provider_name=None):
            inp = (request.stdin or "").strip()
            if inp == "2 3":
                out = "5\n"
            elif inp == "10 20":
                out = "30\n"
        ...
        monkeypatch.setattr(CodeExecutionService, 'execute', _mock_execute)
