...
class TestAttendanceAPIAndSecurity:
    ...
    def test_admin_can_export_csv(
        ...
        api_client.force_authenticate(user=admin_user)
        res = api_client.get(f'/api/v1/admin/assessments/{published_assessment.id}/attendance/export/?format=csv')
        assert res.status_code == status.HTTP_200_OK
        assert 'text/csv' in res['Content-Type']
        assert f'attendance_{published_assessment.id}.csv' in res['Content-Disposition']
        content = res.content.decode('utf-8')
        assert "Student Name,Roll Number,Email,Section" in content
        assert student_user_1.email in content
