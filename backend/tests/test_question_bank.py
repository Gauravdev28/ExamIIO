...
def test_hard_delete_rejected_for_questions_with_published_or_historical_versions(admin_user):
    """34. Questions used in assessments reject hard deletion."""
    from apps.assessments.models import Assessment, AssessmentQuestion, AssessmentStatus
    ...
    question, v1 = QuestionService.create_question(
        ...
        description="Desc",
        points=10,
        type_config={"options": [{"id": "A", "text": "1", "is_correct": True}, {"id": "B", "text": "2", "is_correct": False}], "correct_options": ["A"]},
        actor=admin_user
    )
    QuestionService.publish_version(version=v1, actor=admin_user)
    ...
    with pytest.raises(DRFValidationError) as excinfo:
        QuestionService.delete_draft_question(question=question, actor=admin_user)
    assert excinfo.value.detail.get('code') == 'HARD_DELETE_BLOCKED'


@pytest.mark.django_db
def test_hard_delete_rejected_via_api_returns_structured_code(api_client, admin_user):
    """35. Attempting to delete an assessment-referenced question via API returns HTTP 400 with HARD_DELETE_BLOCKED code."""
    from apps.assessments.models import Assessment, AssessmentQuestion, AssessmentStatus
    from datetime import timedelta
    from django.utils import timezone
    ...
    url = reverse('questions:admin-question-detail', kwargs={'pk': question.id})
    res = api_client.delete(url)
    assert res.status_code == 400
    assert res.json()['error']['code'] == 'HARD_DELETE_BLOCKED'
