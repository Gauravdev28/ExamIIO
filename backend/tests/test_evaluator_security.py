...
from apps.evaluator.services import CodeSubmissionService, Judge0Adapter, PistonAdapter


@pytest.mark.django_db
@pytest.mark.skipif(
    not Judge0Adapter.check_health() and not PistonAdapter.check_health(),
    reason="Live sandbox execution container is unavailable in this test environment."
)
class TestAdversarialSecurityAndSandbox:
    ...
    def test_03_memory_bomb_triggers_mle(self):
        code = "memory_bomb = 'a' * (500 * 1024 * 1024)"
        sub = self._execute_code(code, language='PYTHON')
        assert sub.verdict in [CodeVerdict.MEMORY_LIMIT_EXCEEDED, CodeVerdict.RUNTIME_ERROR, CodeVerdict.TIME_LIMIT_EXCEEDED]

    def test_04_fork_bomb_triggers_runtime_error(self):
        """Process / Fork Bomb: verifies cgroup pids.max ceiling enforcement."""
        code = "import os\nwhile True: os.fork()"
        sub = self._execute_code(code, language='PYTHON')
        assert sub.verdict in [CodeVerdict.RUNTIME_ERROR, CodeVerdict.TIME_LIMIT_EXCEEDED]
        res = sub.test_case_results.first()
        err = res.error_message or ""
        assert any(w in err for w in ["pids.max", "Resource temporarily unavailable", "BlockingIOError", "cannot fork", "Out of memory"]) or sub.verdict == CodeVerdict.TIME_LIMIT_EXCEEDED
    ...
    def test_05_thread_bomb_triggers_runtime_error(self):
        ...
        code = "import threading, time\nwhile True: threading.Thread(target=lambda: time.sleep(10)).start()"
        sub = self._execute_code(code, language='PYTHON')
        assert sub.verdict in [CodeVerdict.RUNTIME_ERROR, CodeVerdict.TIME_LIMIT_EXCEEDED, CodeVerdict.WRONG_ANSWER]
    ...
    def test_06_host_filesystem_access_blocked(self):
        ...
        res = sub.test_case_results.first()
        err = res.error_message or ""
        assert any(w in err for w in ["Permission denied", "No such file", "Operation not permitted"])

    def test_07_proc_inspection_blocked(self):
        """/proc Inspection: verifies PID namespace isolation blocks host /proc access."""
        code = "open('/proc/kcore', 'r')"
        sub = self._execute_code(code, language='PYTHON')
        assert sub.verdict == CodeVerdict.RUNTIME_ERROR
        res = sub.test_case_results.first()
        err = res.error_message or ""
        assert any(w in err for w in ["Permission denied", "No such file", "PID namespace", "Operation not permitted"])
    ...
    def test_10_internet_outbound_access_blocked(self):
        ...
        res = sub.test_case_results.first()
        err = res.error_message or ""
        assert any(w in err for w in ["Network is unreachable", "CLONE_NEWNET", "Temporary failure in name resolution", "timed out", "Connection refused"]) or sub.verdict == CodeVerdict.TIME_LIMIT_EXCEEDED
    ...
    def test_15_privilege_escalation_blocked(self):
        ...
        res = sub.test_case_results.first()
        err = res.error_message or ""
        assert any(w in err for w in ["Operation not permitted", "Permission denied", "Read-only file system"])

    def test_16_dangerous_syscall_blocked(self):
        """Dangerous Syscall Probe: verifies Seccomp-BPF blocks unauthorized syscalls."""
        code = "import ctypes\nctypes.CDLL(None).syscall(314)"
        sub = self._execute_code(code, language='PYTHON')
        assert sub.verdict in [CodeVerdict.RUNTIME_ERROR, CodeVerdict.WRONG_ANSWER]

    def test_17_stdout_flooding_triggers_output_limit(self):
        """Stdout Buffer Flooding: verifies output buffer ceiling truncates or handles large output."""
        code = "import sys\nfor _ in range(1000): sys.stdout.write('A' * 100)"
        sub = self._execute_code(code, language='PYTHON')
        assert sub.verdict in [CodeVerdict.OUTPUT_LIMIT_EXCEEDED, CodeVerdict.TIME_LIMIT_EXCEEDED, CodeVerdict.WRONG_ANSWER]
