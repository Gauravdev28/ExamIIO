        ...
        for idx, row in df.iterrows():
            ...
            if raw_type == QuestionType.MCQ:
                ...
                if raw_langs:
                    ...
                    if invalid_langs:
                        row_errors.append(f"Invalid coding language(s): {', '.join(invalid_langs)}. Supported: PYTHON, CPP, JAVA, C")
                else:
                    langs = [CodingLanguage.PYTHON, CodingLanguage.CPP, CodingLanguage.JAVA, CodingLanguage.C]

                starter_code = str(row.get('starter_code', '')).strip()
                constraints = str(row.get('constraints', '')).strip()

                # Option A: Synchronize allowed languages with starter code
                starter_codes = {}
                if starter_code:
                    starter_codes[CodingLanguage.PYTHON] = starter_code
                    # Option A: only enable language that has starter code
                    langs = [CodingLanguage.PYTHON]

                sample_in = str(row.get('sample_input', '')).strip()
                sample_out = str(row.get('sample_output', '')).strip()
                hidden_in = str(row.get('hidden_input', '')).strip()
                hidden_out = str(row.get('hidden_output', '')).strip()
                ...
                if sample_in != "" or sample_out != "":
                    test_cases.append({
                        ...
                        'points': 5,
                        'is_hidden': False,
                        'is_verified': bool(sample_out and sample_out.upper() not in ('TODO', 'TBD', '?', '[PLACEHOLDER]')),
                        'execution_order': tc_order
                    })
                    tc_order += 1
                ...
                if hidden_in != "" or hidden_out != "":
                    test_cases.append({
                        'name': 'Hidden Case 1',
                        'input_data': hidden_in,
                        'expected_output': hidden_out,
                        'points': 5,
                        'is_hidden': True,
                        'is_verified': bool(hidden_out and hidden_out.upper() not in ('TODO', 'TBD', '?', '[PLACEHOLDER]')),
                        'execution_order': tc_order
                    })
                    tc_order += 1

                # Safe candidate examples (Priority 1: public sample test cases; never hidden)
                examples = []
                ...
                # Point sum normalization with safety condition (P >= N)
                n_tests = len(test_cases)
                if n_tests > 0:
                    if points < n_tests:
                        row_errors.append(
                            f"Question has {points} points but {n_tests} scored test cases require at least 1 point each. Increase question points or reduce scored test cases."
                        )
                    else:
                        base = points // n_tests
                        rem = points % n_tests
