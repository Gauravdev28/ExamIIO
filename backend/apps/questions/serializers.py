...
class QuestionVersionAdminDetailSerializer(serializers.ModelSerializer):
    ...
    def get_is_editable(self, obj: QuestionVersion) -> bool:
        return obj.status == VersionStatus.DRAFT

    def get_is_deletable(self, obj: QuestionVersion) -> bool:
        from .services import QuestionService
        usage = QuestionService.get_question_usage(obj.question)
        return usage.get("is_deletable", False)
...
class QuestionListSerializer(serializers.ModelSerializer):
    ...
    def get_published_version(self, obj: Question):
        v = obj.versions.filter(status=VersionStatus.PUBLISHED).order_by('-version_number').first()
        return QuestionVersionSummarySerializer(v).data if v else None
...
class CreateQuestionSerializer(serializers.Serializer):
    ...
    title = serializers.CharField(max_length=255, required=False, default="Untitled Question", allow_blank=True)
