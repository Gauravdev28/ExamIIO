...
class Question(UUIDModel, TimeStampedModel):
    ...
    def delete(self, *args, **kwargs):
        ...
        for v in self.versions.all():
            v._is_deleting = True
            if hasattr(v, 'coding_config') and v.coding_config:
                v.coding_config._is_deleting = True
                for tc in v.coding_config.test_cases.all():
                    tc._is_deleting = True
            if hasattr(v, 'sql_config') and v.sql_config:
                v.sql_config._is_deleting = True
        return super().delete(*args, **kwargs)


class QuestionVersionQuerySet(models.QuerySet):
    """
    Custom QuerySet enforcing server-level immutability for QuestionVersion records.
    Prevents direct bulk mutations (update, delete) on published or archived versions.
    """
    def update(self, **kwargs):
        if self.filter(status__in=[VersionStatus.PUBLISHED, VersionStatus.ARCHIVED]).exists():
            allowed_mutation_keys = {'status', 'published_at', 'updated_at'}
            attempted_keys = set(kwargs.keys())
            if not attempted_keys.issubset(allowed_mutation_keys):
                raise PermissionDenied("Published question versions are immutable and cannot be mutated via QuerySet.update().")
            if 'status' in kwargs and kwargs['status'] not in [VersionStatus.ARCHIVED, VersionStatus.PUBLISHED]:
                raise PermissionDenied("Published question versions cannot be reverted via QuerySet.update().")
        return super().update(**kwargs)

    def delete(self):
        if self.filter(status__in=[VersionStatus.PUBLISHED, VersionStatus.ARCHIVED]).exists():
            raise PermissionDenied("Published or archived question versions are immutable and cannot be deleted via QuerySet.delete().")
        return super().delete()
...
class QuestionVersion(UUIDModel, TimeStampedModel):
    ...
    """
    objects = QuestionVersionQuerySet.as_manager()
    ...
    def delete(self, *args, **kwargs):
        ...
        if (
            self.assessment_associations.exists()
            or AssessmentSnapshotQuestion.objects.filter(question_version=self).exists()
            or AttemptAnswer.objects.filter(snapshot_question__question_version=self).exists()
            or LegalHold.objects.filter(attempt__assessment__assessment_questions__question_version=self, status=LegalHoldStatus.ACTIVE).exists()
        ):
            raise PermissionDenied(
                f"Cannot delete question version in '{self.status}' status that is used by assessments or examination snapshots."
            )
        is_parent_deleting = getattr(self, '_is_deleting', False) or getattr(getattr(self, 'question', None), '_is_deleting', False)
        if self.status in [VersionStatus.PUBLISHED, VersionStatus.ARCHIVED] and not is_parent_deleting:
            raise PermissionDenied("Published or archived question versions cannot be deleted directly.")
...
class CodingQuestionConfig(UUIDModel, TimeStampedModel):
    ...
    def delete(self, *args, **kwargs):
        from apps.assessments.models import AssessmentQuestion, AssessmentSnapshotQuestion, AttemptAnswer
        if (
            self.question_version.assessment_associations.exists()
            or AssessmentSnapshotQuestion.objects.filter(question_version=self.question_version).exists()
            or AttemptAnswer.objects.filter(snapshot_question__question_version=self.question_version).exists()
        ):
            raise PermissionDenied("Cannot delete coding configuration belonging to a version used by assessments.")
        is_parent_deleting = getattr(self, '_is_deleting', False) or getattr(self.question_version, '_is_deleting', False) or getattr(getattr(self.question_version, 'question', None), '_is_deleting', False)
        if self.question_version.status in [VersionStatus.PUBLISHED, VersionStatus.ARCHIVED] and not is_parent_deleting:
            raise PermissionDenied("Coding configurations belonging to a published/archived version are immutable and cannot be deleted directly.")
        super().delete(*args, **kwargs)
...
class TestCase(UUIDModel, TimeStampedModel):
    ...
    def delete(self, *args, **kwargs):
        from apps.assessments.models import AssessmentQuestion, AssessmentSnapshotQuestion, AttemptAnswer
        qv = self.coding_config.question_version
        if (
            qv.assessment_associations.exists()
            or AssessmentSnapshotQuestion.objects.filter(question_version=qv).exists()
            or AttemptAnswer.objects.filter(snapshot_question__question_version=qv).exists()
        ):
            raise PermissionDenied("Cannot delete test case belonging to a version used by assessments.")
        is_parent_deleting = getattr(self, '_is_deleting', False) or getattr(self.coding_config, '_is_deleting', False) or getattr(qv, '_is_deleting', False) or getattr(getattr(qv, 'question', None), '_is_deleting', False)
        if qv.status in [VersionStatus.PUBLISHED, VersionStatus.ARCHIVED] and not is_parent_deleting:
            raise PermissionDenied("Test cases belonging to a published/archived version are immutable and cannot be deleted directly.")
        super().delete(*args, **kwargs)
...
class SQLQuestionConfig(UUIDModel, TimeStampedModel):
    ...
    def delete(self, *args, **kwargs):
        ...
        if (
            ...
            raise PermissionDenied("Cannot delete SQL configuration belonging to a version used by assessments.")
        is_parent_deleting = getattr(self, '_is_deleting', False) or getattr(self.question_version, '_is_deleting', False) or getattr(getattr(self.question_version, 'question', None), '_is_deleting', False)
        if self.question_version.status in [VersionStatus.PUBLISHED, VersionStatus.ARCHIVED] and not is_parent_deleting:
            raise PermissionDenied("SQL configurations belonging to a published/archived version are immutable and cannot be deleted directly.")
        super().delete(*args, **kwargs)
