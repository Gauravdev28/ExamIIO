...
class LiveInterventionService:
    ...
    @classmethod
    def cancel_pending_termination(
        cls,
        proctor: User,
        attempt_id: str,
        reason: str = '',
        ...
        with transaction.atomic():
            ...
            if attempt.status != AttemptStatus.IN_PROGRESS:
                raise DRFValidationError({"detail": f"Cannot cancel termination for attempt in status {attempt.status}."})

            if not attempt.termination_pending:
                raise DRFValidationError({"detail": "Attempt does not have a pending termination."})

            # Authoritative deadline expiration check
            if attempt.termination_deadline and now >= attempt.termination_deadline:
                ...
                AttemptTerminationPolicyService._execute_expiration_locked(attempt)
                is_expired = True
            else:
                # Cancel pending termination
                attempt.termination_pending = False
                attempt.termination_cancelled_at = now
                attempt.termination_cancelled_by = proctor
                attempt.state_version = models.F('state_version') + 1
                attempt.save(update_fields=['termination_pending', 'termination_cancelled_at', 'termination_cancelled_by', 'state_version', 'updated_at'])
                ...
                intervention = ProctorIntervention.objects.create(
                    ...
                    reason_code="PROCTOR_TERMINATION_CANCELLED",
                    reason_text=reason or "Proctor intervened and cancelled pending examination termination.",
                    internal_notes=internal_notes,
                    request_idempotency_key=idempotency_key,
                    metadata={
                ...
                payload = {
                    ...
                    "message": "Proctor has cancelled the pending termination. You may resume your examination.",
        ...
        if is_expired:
            raise DRFValidationError({"detail": "Cannot cancel termination: 2-minute deadline has expired."})
...
class ProctorTriageQueueService:
    ...
    def get_triage_roster(cls, assessment_id: str, proctor_user: Optional[User] = None) -> List[Dict[str, Any]]:
        ...
        # Fetch all IN_PROGRESS attempts and DISQUALIFIED attempts for this assessment
        attempts = TestAttempt.objects.filter(
            assessment=assessment
        ).filter(
            Q(status=AttemptStatus.IN_PROGRESS) | Q(is_disqualified=True)
        ...
        for att in attempts:
            # Check if pending termination has expired authoritatively
            if att.termination_pending and att.termination_deadline and now >= att.termination_deadline:
                from apps.proctoring.services import AttemptTerminationPolicyService
                AttemptTerminationPolicyService.check_and_expire_termination(str(att.id))
                att.refresh_from_db()
            ...
            roster_items.append({
                ...
                "status": att.status,
                "attempt_status": "DISQUALIFIED" if att.is_disqualified else att.status,
                "is_disqualified": att.is_disqualified,
                ...
                "remaining_seconds": remaining_seconds if att.status == AttemptStatus.IN_PROGRESS else 0,
                ...
                "priority_rank": 0 if att.is_disqualified else cls.BAND_PRIORITY.get(risk_band, 99),
                "termination_pending": att.termination_pending,
