...
class ProctorCancelTerminationView(APIView):
    """
    Cancels pending 2-minute examination termination if deadline has not expired.
    Rescues the attempt, keeping it IN_PROGRESS, preserving answers and original expires_at.
    """
    permission_classes = [IsAuthenticated, IsProctorOrAdmin, HasAttemptInvigilationAccess]
    ...
    def post(self, request, attempt_id):
        ...
        return Response({
            ...
            "attempt_status": attempt.status,
            "cancelled_at": attempt.termination_cancelled_at.isoformat() if attempt.termination_cancelled_at else None,
            "cancelled_by": attempt.termination_cancelled_by.email if attempt.termination_cancelled_by else None,
            "intervention_id": str(intervention.id) if intervention else None,
            "message": "Termination cancelled successfully. Attempt remains in progress."
        }, status=status.HTTP_200_OK)
