...
class TestAttemptConsumer(AsyncJsonWebsocketConsumer):
    ...
    async def connect(self):
        ...
        cache.delete(f"ws_disconnect_{self.attempt_id}")
    ...
    async def disconnect(self, close_code):
        ...
        if hasattr(self, 'attempt_id'):
            await self._schedule_disconnect_countdown()
    ...
    async def proctor_event(self, event):
        """
        Handler for real-time invigilation interventions and proctoring warnings.
        """
    ...
    @database_sync_to_async
    def _schedule_disconnect_countdown(self):
        import uuid
        import time
        from django.core.cache import cache
        from .models import TestAttempt, AttemptStatus
        ...
        disconnect_token = str(uuid.uuid4())
        cache.set(f"ws_disconnect_{self.attempt_id}", {
            "token": disconnect_token,
            "disconnected_at": time.time(),
        }, timeout=60)
        from apps.proctoring.tasks import check_websocket_disconnect_task
        check_websocket_disconnect_task.apply_async(args=[str(self.attempt_id), disconnect_token], countdown=10)
