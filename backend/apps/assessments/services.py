        ...
        for aq in assessment_questions:
            ...
            if qv.question_type == QuestionType.CODING and hasattr(qv, 'coding_config') and qv.coding_config:
                ...
                public_coding_config = {
                    ...
                    "starter_codes": c_conf.starter_codes or {},
                    "examples": c_conf.examples or [],
            ...
            # Sanitize type_config for student presentation (strictly strip answer keys)
            student_type_config = {}
            if qv.type_config and isinstance(qv.type_config, dict):
                import copy
                student_type_config = copy.deepcopy(qv.type_config)
                for secret_key in [
                    'correct_option',
                    'correct_options',
                    'correct_answer',
                    ...
                    student_type_config.pop(secret_key, None)

                # Strictly strip 'is_correct' from individual options
                if 'options' in student_type_config and isinstance(student_type_config['options'], list):
                    sanitized_options = []
                    for opt in student_type_config['options']:
                        if isinstance(opt, dict):
                            opt_copy = copy.deepcopy(opt)
                            opt_copy.pop('is_correct', None)
                            sanitized_options.append(opt_copy)
                        else:
                            sanitized_options.append(opt)
                    student_type_config['options'] = sanitized_options
            ...
            snap_q = AssessmentSnapshotQuestion.objects.create(
                ...
                type_config=student_type_config,
...
def resolve_student_users(student_ids: Any, active_only: bool = False) -> List[User]:
    """
    Centralized student identity resolver for the assessment domain.
    Resolves canonical User.id, legacy/API StudentProfile.id, student email, and roll number.
    ...
    for sid in clean_ids:
        try:
            ...
            text_tokens.append(sid)

    candidate_filter = Q(role=Role.STUDENT)
    if active_only:
        candidate_filter &= Q(is_active=True)
    if StudentProfile.objects.filter(user__role=Role.STUDENT).exists():
        candidate_filter &= Q(student_profile__isnull=False)

    token_filter = Q()
    if valid_uuid_ids:
        token_filter |= Q(id__in=valid_uuid_ids) | Q(student_profile__id__in=valid_uuid_ids)
    if text_tokens:
        token_filter |= Q(email__in=text_tokens) | Q(student_profile__roll_number__in=text_tokens)

    candidate_users = list(
        User.objects.filter(candidate_filter & token_filter)
    ...
    for sid in clean_ids:
        ...
        for u in candidate_users:
            ...
            roll = profile.roll_number.lower() if (profile and profile.roll_number) else None

            if (
                str(u.id) == sid
                or profile_id == sid
                or u.email.lower() == sid_lower
                or (roll and roll == sid_lower)
            ):
                matched_user = u
                break
    ...
    if invalid_ids:
        raise DRFValidationError({
            ...
            "students": f"Account(s) {', '.join(invalid_ids)} cannot be targeted as a student (must be student accounts in Craft Society).",
...
class AssessmentAudienceService:
    """
    Authoritative domain service for resolving and configuring assessment candidate audience targeting.
    The active product is SECTION-FREE. Candidates are targeted via ALL_STUDENTS or SELECTED_STUDENTS.
    AssessmentAssignment is the authoritative access control relationship.
    """
    @classmethod
    def resolve_audience(
        ...
        if student_ids is None:
            target_ids = set(assessment.target_students.filter(student_profile__isnull=False).values_list('id', flat=True))
            assigned_ids = set(assessment.assignments.filter(status=AssignmentStatus.ASSIGNED, student__student_profile__isnull=False).values_list('student_id', flat=True))
            student_ids = [str(uid) for uid in target_ids.union(assigned_ids)]
        else:
            student_ids = [str(uid) for uid in student_ids if uid]

        # 1. Resolve Section Students (legacy support; active product is section-free)
        ...
        individual_students = resolve_student_users(student_ids, active_only=False)
        ...
        total_craft_society_students = StudentService.get_canonical_students_queryset(active_only=False).count()
        is_all_targeted = (
            total_eligible > 0
            and total_eligible == total_craft_society_students
            and len(section_student_ids) == 0
        )
        ...
        return {
            ...
            "total_craft_society_students": total_craft_society_students,
            "all_students_count": total_craft_society_students,
            "eligible_student_ids": final_eligible_ids,
            "target_all_students": is_all_targeted,
            "audience_mode": "ALL_STUDENTS" if is_all_targeted else "SPECIFIC",
    ...
    def configure_audience(
        ...
        """
        Configures draft assessment candidate audience.
        Supports All Students (every student in Craft Society) or explicitly selected candidates.
        Active product operates section-free via ALL_STUDENTS or SELECTED_STUDENTS.
        ...
        raw_student_ids = [str(uid) for uid in student_ids if uid] if student_ids is not None else []

        is_all_students = target_all_students or any(s.upper() in ['ALL', 'ALL_STUDENTS', 'ALL_ACTIVE'] for s in raw_student_ids)

        if is_all_students:
            students = list(StudentService.get_canonical_student_users_queryset(active_only=False))
            sections = []
        else:
            # Validate sections exist and are active (if any provided)
            ...
            students = resolve_student_users(raw_student_ids, active_only=False)

        with transaction.atomic():
            if is_all_students or section_ids is not None:
            ...
            AuditService.log(
                ...
                metadata={
                    "title": assessment.title,
                    "target_mode": "ALL_STUDENTS" if is_all_students else ("SECTIONS_AND_STUDENTS" if sections else "SELECTED_STUDENTS"),
...
class AssessmentService:
    ...
    def create_assessment(
        ...
        **kwargs
        ...
        proctoring_enabled = kwargs.get('proctoring_enabled', True)
        camera_required = kwargs.get('camera_required', True)
        phone_detection_enabled = kwargs.get('phone_detection_enabled', True)
        face_detection_enabled = kwargs.get('face_detection_enabled', True)
        multiple_face_detection_enabled = kwargs.get('multiple_face_detection_enabled', True)
        gaze_detection_enabled = kwargs.get('gaze_detection_enabled', True)
        head_movement_detection_enabled = kwargs.get('head_movement_detection_enabled', True)
        max_confirmed_violations = kwargs.get('max_confirmed_violations', 3)

        if max_confirmed_violations is not None and max_confirmed_violations < 3:
            raise DRFValidationError({"max_confirmed_violations": "Minimum confirmed violation threshold is strictly 3."})
        ...
        assessment = Assessment.objects.create(
            ...
            multiple_face_detection_enabled=multiple_face_detection_enabled,
            gaze_detection_enabled=gaze_detection_enabled,
            head_movement_detection_enabled=head_movement_detection_enabled,
            max_confirmed_violations=max_confirmed_violations,
    ...
    def update_draft_assessment(
        ...
        **kwargs
        ...
        if is_published:
            if target_section_ids is not None or target_student_ids is not None or target_all_students is not None:
        ...
        for f in [
            'proctoring_enabled', 'camera_required', 'phone_detection_enabled',
            'face_detection_enabled', 'multiple_face_detection_enabled',
            'gaze_detection_enabled', 'head_movement_detection_enabled'
        ]:
            if f in kwargs and kwargs[f] is not None:
                setattr(assessment, f, kwargs[f])
        if 'max_confirmed_violations' in kwargs and kwargs['max_confirmed_violations'] is not None:
            max_v = kwargs['max_confirmed_violations']
            if max_v < 3:
                raise DRFValidationError({"max_confirmed_violations": "Minimum confirmed violation threshold is strictly 3."})
            assessment.max_confirmed_violations = max_v
        ...
        if not is_published and (target_section_ids is not None or target_student_ids is not None or target_all_students is not None):
            ...
            is_all = target_all_students if target_all_students is not None else False
            AssessmentAudienceService.configure_audience(
                assessment=assessment,
                section_ids=sec_ids,
                student_ids=stu_ids,
                target_all_students=is_all,
                actor=actor,
                request=request
            )
    ...
    def add_question(
        ...
        if question_version.status != VersionStatus.PUBLISHED:
            raise DRFValidationError(
                {"question_version": f"Only published question versions can be added to an assessment (version status is {question_version.status})."}
            )
    ...
    def add_question_to_assessment(
        cls,
        assessment_or_id: Any = None,
        question_version_or_id: Any = None,
        actor: Optional[User] = None,
        order: Optional[int] = None,
        points: Optional[int] = None,
        negative_marking_enabled: bool = False,
        ...
        **kwargs
        ...
        a_id = assessment_or_id if assessment_or_id is not None else (kwargs.get('assessment_id') or kwargs.get('assessment'))
        q_id = question_version_or_id if question_version_or_id is not None else (kwargs.get('question_version_id') or kwargs.get('question_version'))
        act = actor or kwargs.get('actor')
        pts = points if points is not None else kwargs.get('points')

        if isinstance(a_id, Assessment):
            assessment = a_id
        else:
            assessment = Assessment.objects.get(id=str(a_id))

        if isinstance(q_id, QuestionVersion):
            question_version = q_id
        else:
            question_version = QuestionVersion.objects.get(id=str(q_id))

        return cls.add_question(
            assessment=assessment,
            question_version=question_version,
            actor=act,
            order=order,
    ...
    def assign_students(
        ...
        resolved_students = resolve_student_users(student_ids, active_only=False)
        if not resolved_students:
            raise DRFValidationError({
                "error": "INVALID_STUDENT_IDS",
                "invalid_ids": student_ids,
                "detail": "None of the requested student IDs could be resolved to students."
            })
...
class AttemptService:
    ...
    @classmethod
    @transaction.atomic
    def start_attempt(cls, student: User, assessment_id: str, actor: Optional[User] = None, request=None) -> Tuple[TestAttempt, bool]:
        """
        Starts or resumes a student test attempt.
        Concurrency-safe via select_for_update and attempt_limit verification.
        Supports Proctor Reattempt / Second-Chance workflow.
        """
        # Lock Assessment
        ...
        # Verify Student Assignment - Concurrency Serialization Anchor
        try:
        ...
        if auth is not None:
            ...
            # If candidate has a CANCELLED attempt, they cannot resume or retake
            for att in existing_attempts:
                if att.status == AttemptStatus.CANCELLED:
                    raise DRFValidationError({"status": "CANDIDATE_DISQUALIFIED: Disqualified attempts cannot be resumed or retaken."})
    ...
    def save_answer(
        ...
        # Window termination check & auto expire
        if attempt.termination_pending and attempt.termination_deadline and timezone.now() >= attempt.termination_deadline:
    ...
    def submit_attempt(cls, student: User, attempt_id: str, actor: Optional[User] = None, request=None) -> TestAttempt:
        ...
        # Terminal cancelled/disqualified attempt cannot be submitted
        if attempt.status == AttemptStatus.CANCELLED or getattr(attempt, 'is_disqualified', False):
            raise DRFValidationError({
                "status": "Cannot submit a cancelled or disqualified test attempt."
            })
        ...
        def _dispatch_finalization(att_id: str, act_user, req_obj):
            try:
                from apps.results.tasks import finalize_assessment_result_task
                finalize_assessment_result_task.delay(att_id)
            except Exception as exc:
                import logging
                logging.getLogger(__name__).warning(
                    f"Celery dispatch failed for attempt {att_id}, falling back to synchronous inline finalization: {exc}"
                )
                try:
                    from apps.results.services import ResultFinalizationService
                    ResultFinalizationService.finalize_attempt(attempt_id=att_id, actor=act_user, request=req_obj)
                except Exception as inline_exc:
                    logging.getLogger(__name__).error(
                        f"Synchronous finalization fallback also failed for attempt {att_id}: {inline_exc}"
                    )

        transaction.on_commit(lambda: _dispatch_finalization(str(attempt.id), actor or student, request))

        def _dispatch_certificate(att_id: str, act_user):
            try:
                from apps.results.tasks import generate_certificate_task
                generate_certificate_task.delay(att_id)
            except Exception as exc:
                import logging
                logging.getLogger(__name__).warning(
                    f"Celery dispatch failed for certificate generation on attempt {att_id}, falling back to inline: {exc}"
                )
                try:
                    from apps.results.certificate_service import CertificateService
                    att = TestAttempt.objects.filter(id=att_id).first()
                    if att:
                        CertificateService.generate_or_get_certificate(attempt=att, actor=act_user)
                except Exception as inline_exc:
                    logging.getLogger(__name__).error(
                        f"Synchronous certificate generation fallback failed for attempt {att_id}: {inline_exc}"
                    )

        transaction.on_commit(lambda: _dispatch_certificate(str(attempt.id), actor or student))

        return attempt

    @classmethod
    @transaction.atomic
    def terminate_attempt_explicitly(
        cls,
        student: User,
        attempt_id: str,
        reason: str = "EXAMINATION ABANDONED — BROWSER NAVIGATION / ROOM EXIT",
        actor: User = None,
        request = None
    ) -> TestAttempt:
        ...
        if attempt.student != student and actor != attempt.student and not (actor and getattr(actor, 'is_staff', False)):
            raise PermissionDenied("You are not authorized to terminate this attempt.")

        # Idempotent return if already cancelled
        if attempt.status == AttemptStatus.CANCELLED:
            return attempt

        # Terminal conflict check: cannot terminate already submitted or expired attempts
        ...
        attempt.save(update_fields=['status', 'is_disqualified', 'disqualification_reason', 'termination_reason', 'updated_at'])

        # Terminate proctoring session if exists
        try:
            from apps.proctoring.models import ProctoringSession, ProctoringSessionStatus, ProctoringEvent, EventSource, EventSeverity
            proct_session = ProctoringSession.objects.filter(attempt=attempt).first()
            if proct_session and proct_session.status != ProctoringSessionStatus.TERMINATED:
                proct_session.status = ProctoringSessionStatus.TERMINATED
                proct_session.save(update_fields=['status', 'updated_at'])

                ProctoringEvent.objects.create(
                    session=proct_session,
                    event_type='EXAM_EXIT_UNAUTHORIZED',
                    source=EventSource.BROWSER,
                    severity=EventSeverity.HIGH,
                    confidence=1.0,
                    is_confirmed=True,
                    started_at=now,
                    metadata={"reason": reason, "exit_type": "EXPLICIT_EXIT"}
                )
        except Exception as e:
            import logging
            logging.getLogger(__name__).warning(f"Could not terminate proctoring session for attempt {attempt.id}: {e}")

        # Record ProctorIntervention
        try:
            ...
            ProctorIntervention.objects.create(
                ...
                reason_code="EXAM_ROOM_ABANDONED",
                reason_text=reason,
                metadata={"terminated_at": now.isoformat()}
            )
        except Exception as e:
            import logging
            logging.getLogger(__name__).warning(f"Could not record ProctorIntervention for attempt {attempt.id}: {e}")
...
class AssessmentAttendanceService:
    ...
    def get_attendance_data(
        ...
        for a in assignments_qs:
            ...
            # - If any attempt was disqualified -> DISQUALIFIED
            # - Else, if any attempt has started_at is not None -> ATTENDED
            # - Else, if candidate attempt was cancelled before start or assessment is cancelled -> NOT_ATTENDED
            # - Else, if assessment.end_datetime has passed -> NOT_ATTENDED
            # - Else (before or during exam window without an attempt) -> NOT_STARTED
            has_disqualified = any(getattr(att, 'is_disqualified', False) for att in attempts)
            has_started = any(att.started_at is not None for att in attempts)
            has_cancelled_before_start = any(att.started_at is None and att.status == AttemptStatus.CANCELLED and not getattr(att, 'is_disqualified', False) for att in attempts)
            now = timezone.now()
            if has_disqualified:
                attendance_status = 'DISQUALIFIED'
            elif has_started:
                attendance_status = 'ATTENDED'
            elif has_cancelled_before_start:
                attendance_status = 'NOT_ATTENDED'
            elif assessment.end_datetime and now >= assessment.end_datetime:
                attendance_status = 'NOT_ATTENDED'
            else:
            ...
            candidate_name = getattr(profile, 'certificate_name', '') or getattr(student, 'display_name', '') or student.email
        ...
        total_not_attended = sum(1 for r in filtered_records if r['attendance_status'] == 'NOT_ATTENDED')
        total_not_started = sum(1 for r in filtered_records if r['attendance_status'] == 'NOT_STARTED')
        total_disqualified = sum(1 for r in filtered_records if r['attendance_status'] == 'DISQUALIFIED')
        ...
        for r in filtered_records:
            ...
            if r['attendance_status'] == 'ATTENDED':
                grp["attended"] += 1
            elif r['attendance_status'] == 'NOT_ATTENDED':
                grp["not_attended"] += 1
            else:
                grp["not_started"] += 1
    ...
    @classmethod
    def export_attendance_csv(cls, assessment: Assessment, filters: Optional[Dict[str, Any]] = None) -> io.StringIO:
        ...
        writer.writerow([_sanitize_formula_injection(h) for h in headers])
