...
class Assessment(UUIDModel, TimeStampedModel):
    ...
    proctoring_enabled = models.BooleanField(
        default=True,
        help_text="Master switch for AI-assisted proctoring and secure browser mode."
    )
    camera_required = models.BooleanField(
        default=True,
        help_text="Requires student webcam access and pre-exam face verification."
    )
    phone_detection_enabled = models.BooleanField(
        default=True,
        help_text="Detects unauthorized mobile phones or devices via vision model."
    )
    face_detection_enabled = models.BooleanField(
        default=True,
        help_text="Monitors face presence and candidate visibility."
    )
    multiple_face_detection_enabled = models.BooleanField(
        default=True,
        help_text="Detects multiple individuals in the camera frame."
    )
    gaze_detection_enabled = models.BooleanField(
        default=True,
        help_text="Tracks eye gaze deviation away from the examination viewport."
    )
    head_movement_detection_enabled = models.BooleanField(
        default=True,
        help_text="Tracks persistent head turns or looking away."
    )
    max_confirmed_violations = models.PositiveIntegerField(
        default=3,
        validators=[MinValueValidator(3)],
        help_text="Minimum confirmed violation threshold is strictly 3."
    )
    ...
    target_sections = models.ManyToManyField(
        ...
        help_text="Academic sections targeted for this assessment (deprecated for active workflows; preserved for historical records)."
    )
    ...
    def clean(self):
        ...
        if self.max_confirmed_violations is not None and self.max_confirmed_violations < 3:
            raise ValidationError({"max_confirmed_violations": "Minimum confirmed violation threshold is strictly 3."})

    def save(self, *args, **kwargs):
        self.clean()
        if not self._state.adding and self.pk:
            original = Assessment.objects.get(pk=self.pk)
            # Immutability check: Archived assessments are completely immutable
            if original.status == AssessmentStatus.ARCHIVED:
                raise PermissionDenied("Archived assessments are permanently immutable.")

            # Immutability check: Published assessments preserve structural integrity
            if original.status == AssessmentStatus.PUBLISHED:
                if self.status == AssessmentStatus.DRAFT:
                    raise PermissionDenied("Cannot revert a published assessment to draft.")

                if self.total_points != original.total_points:
                    raise PermissionDenied("Cannot modify total points of a published assessment.")

                if self.passing_percentage != original.passing_percentage:
                    raise PermissionDenied("Cannot modify passing percentage of a published assessment.")

                if original.attempts.exists():
                    if self.start_datetime != original.start_datetime:
                        raise PermissionDenied("Cannot modify start datetime after student attempts have started.")
                    if self.duration_minutes != original.duration_minutes:
                        raise PermissionDenied("Cannot modify duration after student attempts have started.")
                    if self.attempt_limit != original.attempt_limit:
                        raise PermissionDenied("Cannot modify attempt limit after student attempts have started.")
...
class TestAttempt(UUIDModel, TimeStampedModel):
    ...
    disqualification_reason = models.CharField(max_length=255, blank=True, default="")
    disqualified_at = models.DateTimeField(null=True, blank=True)
    termination_pending = models.BooleanField(default=False, db_index=True)
    termination_deadline = models.DateTimeField(null=True, blank=True, db_index=True)
    termination_reason = models.CharField(max_length=255, blank=True, default="")
    termination_cancelled_at = models.DateTimeField(null=True, blank=True)
    termination_cancelled_by = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        null=True,
        blank=True,
        on_delete=models.SET_NULL,
        related_name='cancelled_terminations'
    )
    state_version = models.PositiveIntegerField(default=0, db_index=True)
    ...
    def clean(self):
        ...
        if not self._state.adding:
            ...
            if original.is_disqualified:
                if not self.is_disqualified:
                    raise PermissionDenied("Disqualified attempts are permanently disqualified.")
                if new_status != AttemptStatus.CANCELLED:
                    raise PermissionDenied("Disqualified attempts must remain in CANCELLED status.")
