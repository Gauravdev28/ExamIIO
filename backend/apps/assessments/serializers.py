...
class AssessmentQuestionAdminSerializer(serializers.ModelSerializer):
    ...
    description = serializers.CharField(source='question_version.description', read_only=True)
    instructions = serializers.CharField(source='question_version.instructions', read_only=True)
    version_number = serializers.IntegerField(source='question_version.version_number', read_only=True)
    type_config = serializers.JSONField(source='question_version.type_config', read_only=True)
    coding_config = serializers.SerializerMethodField()
    sql_config = serializers.SerializerMethodField()
    ...
    def get_coding_config(self, obj: AssessmentQuestion):
        qv = obj.question_version
        if qv and hasattr(qv, 'coding_config') and qv.coding_config:
            from apps.questions.serializers import CodingQuestionConfigAdminSerializer
            return CodingQuestionConfigAdminSerializer(qv.coding_config).data
        return None

    def get_sql_config(self, obj: AssessmentQuestion):
        qv = obj.question_version
        if qv and hasattr(qv, 'sql_config') and qv.sql_config:
            from apps.questions.serializers import SQLQuestionConfigSerializer
            return SQLQuestionConfigSerializer(qv.sql_config).data
        return None
...
class AssessmentAdminListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'head_movement_detection_enabled',
            'max_confirmed_violations',
            'question_count',
            'assigned_count',
...
class ConfigureAudienceSerializer(serializers.Serializer):
    target_all_students = serializers.BooleanField(required=False, default=False)
    ...
    studentIds = serializers.ListField(child=serializers.CharField(), required=False)
    target_section_ids = serializers.ListField(child=serializers.UUIDField(), required=False)
    target_student_ids = serializers.ListField(child=serializers.CharField(), required=False)
    targetSectionIds = serializers.ListField(child=serializers.UUIDField(), required=False)
    targetStudentIds = serializers.ListField(child=serializers.CharField(), required=False)
...
class CreateAssessmentSerializer(serializers.Serializer):
    ...
    proctoring_enabled = serializers.BooleanField(required=False, default=True)
    camera_required = serializers.BooleanField(required=False, default=True)
    phone_detection_enabled = serializers.BooleanField(required=False, default=True)
    face_detection_enabled = serializers.BooleanField(required=False, default=True)
    multiple_face_detection_enabled = serializers.BooleanField(required=False, default=True)
    gaze_detection_enabled = serializers.BooleanField(required=False, default=True)
    head_movement_detection_enabled = serializers.BooleanField(required=False, default=True)
    max_confirmed_violations = serializers.IntegerField(min_value=3, required=False, default=3)
...
class UpdateAssessmentSerializer(serializers.Serializer):
    ...
    proctoring_enabled = serializers.BooleanField(required=False)
    camera_required = serializers.BooleanField(required=False)
    phone_detection_enabled = serializers.BooleanField(required=False)
    face_detection_enabled = serializers.BooleanField(required=False)
    multiple_face_detection_enabled = serializers.BooleanField(required=False)
    gaze_detection_enabled = serializers.BooleanField(required=False)
    head_movement_detection_enabled = serializers.BooleanField(required=False)
    max_confirmed_violations = serializers.IntegerField(min_value=3, required=False)
    target_all_students = serializers.BooleanField(required=False, allow_null=True)
    ...
    def validate(self, attrs):
        ...
        start = attrs.get('start_datetime') or (inst.start_datetime if inst else None)
        end = attrs.get('end_datetime') or (inst.end_datetime if inst else None)
        if start and end and end <= start:
            raise serializers.ValidationError({"end_datetime": "End datetime must be strictly after start datetime."})
...
class AddQuestionToAssessmentSerializer(serializers.Serializer):
    question_version_id = serializers.UUIDField(required=False)
    ...
    def validate(self, attrs):
        if not attrs.get('question_version_id') and not attrs.get('question_id'):
            raise serializers.ValidationError({"question_version_id": "Either question_version_id or question_id is required."})
        return attrs
