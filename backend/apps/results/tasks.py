...
@shared_task(
    ...
    max_retries=3,
    default_retry_delay=5,
    name='apps.results.tasks.generate_certificate_task'
)
def generate_certificate_task(self, attempt_id: str):
    """
    Asynchronously generates participation certificate for a submitted attempt.
    """
    try:
        ...
        if not attempt:
            logger.warning(f"TestAttempt {attempt_id} not found for certificate generation.")
            return None
        cert = CertificateService.generate_or_get_certificate(attempt=attempt)
        if cert:
            logger.info(f"Certificate {cert.certificate_id} generated for attempt {attempt_id}")
            return str(cert.id)
        return None
    except Exception as exc:
        logger.error(f"Error generating certificate for attempt {attempt_id}: {exc}", exc_info=True)
        raise self.retry(exc=exc)
