...
class CertificateQuerySet(models.QuerySet):
    def update(self, **kwargs):
        ...
        attempted = set(kwargs.keys()) & blocked_fields
        if attempted:
            raise PermissionDenied(
                f"Direct query update on immutable certificate fields {attempted} is forbidden."
            )
        return super().update(**kwargs)

    def delete(self):
        raise PermissionDenied("Bulk deletion of Certificate records is permanently forbidden.")


class Certificate(UUIDModel, TimeStampedModel):
    """
    Authoritative Participation Certificate record for Craft Society examinations.
    Strict immutability applies to (exam, student, attempt, certificate_id, printed_name)
    immediately upon creation across all lifecycle statuses (PENDING, FAILED, ISSUED, REVOKED).
    """
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=['exam', 'student'],
                name='unique_certificate_exam_student'
            )
        ]
        indexes = [
            models.Index(fields=['student', '-created_at'], name='idx_cert_student'),
            models.Index(fields=['exam', 'status'], name='idx_cert_exam_status'),
        ]

    def __str__(self):
        return f"Certificate {self.certificate_id} - {self.student.email} on {self.exam.title} ({self.status})"

    def clean(self):
        super().clean()
        if not self._state.adding and self.pk:
            original = Certificate.objects.filter(pk=self.pk).first()
            if original:
                immutable_fields = [
                    ('exam_id', original.exam_id, self.exam_id),
                    ('student_id', original.student_id, self.student_id),
                    ('attempt_id', original.attempt_id, self.attempt_id),
                    ('certificate_id', original.certificate_id, self.certificate_id),
                    ('printed_name', original.printed_name, self.printed_name),
                ]
                for fname, old_val, new_val in immutable_fields:
                    if old_val != new_val:
                        raise PermissionDenied(
                            f"Certificate field {fname} is permanently immutable after creation."
                        )
    ...
    def delete(self, *args, **kwargs):
        raise PermissionDenied("Certificate records are permanently immutable and cannot be deleted.")


class StudentCoinLedger(UUIDModel, TimeStampedModel):
    """
    Append-only immutable ledger tracking student coin rewards.
    Awards strictly +3 coins per correctly answered question upon finalized evaluation.
    Enforces attempt-aware idempotency via UniqueConstraint(fields=['attempt', 'question_id']).
    """
    student = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.CASCADE,
    ...
    reason = models.CharField(max_length=255, default="Correct question evaluation")
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                ...
                name='unique_attempt_question_coins'
            )
        ]
        indexes = [
            models.Index(fields=['student', '-awarded_at'], name='idx_coin_student_awarded'),
            models.Index(fields=['attempt', 'question_id'], name='idx_coin_attempt_qid'),
        ]

    def __str__(self):
        return f"+{self.coins_awarded} coins -> {self.student.email} (Attempt: {self.attempt_id}, Q: {self.question_id})"

    def save(self, *args, **kwargs):
        if not self._state.adding and self.pk:
            raise PermissionDenied("StudentCoinLedger entries are strictly append-only and permanently immutable.")
        super().save(*args, **kwargs)

    def delete(self, *args, **kwargs):
        raise PermissionDenied("StudentCoinLedger entries are strictly append-only and cannot be deleted.")
