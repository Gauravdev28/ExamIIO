...
class CoinRewardService:
    """
    Authoritative student coin reward domain service.
    Rules:
    - Strictly awards +3 coins per correctly answered question upon finalized evaluation.
    - Attempt-aware idempotency via UniqueConstraint on (attempt, question_id).
    - Immutable ledger entries; total balance is sum(coins_awarded).
    - Deterministic leaderboard and milestone progress calculation.
    """
    COINS_PER_CORRECT_QUESTION = 3
    MILESTONES = [0, 50, 100, 250, 500]

    @classmethod
    def award_coins_for_finalized_attempt(cls, attempt: TestAttempt, question_results: List[QuestionResult]) -> int:
        """
        Awards strictly +3 coins for each correctly answered question in a finalized attempt.
        Idempotent: uses get_or_create with (attempt, question_id).
        Returns total newly awarded coins.
        """
        from apps.results.models import StudentCoinLedger
        newly_awarded = 0
        for qr in question_results:
            if qr.is_correct:
                _, created = StudentCoinLedger.objects.get_or_create(
                    attempt=attempt,
                    question_id=qr.question_id,
                    defaults={
                        'student': attempt.student,
                        'coins_awarded': cls.COINS_PER_CORRECT_QUESTION,
                        'reason': f"Correct answer on {attempt.assessment.title}"
                    }
                )
                if created:
                    newly_awarded += cls.COINS_PER_CORRECT_QUESTION
        return newly_awarded

    @classmethod
    def get_student_coin_summary(cls, user: User) -> Dict[str, Any]:
        """
        Computes total coins, milestone progress, and recent coin ledger history.
        Milestones: 0 -> 50 -> 100 -> 250 -> 500
        """
        from apps.results.models import StudentCoinLedger
        from django.db.models import Sum

        total_coins = StudentCoinLedger.objects.filter(student=user).aggregate(
            total=Sum('coins_awarded')
        )['total'] or 0

        current_milestone = 0
        next_milestone = cls.MILESTONES[-1]
        ...
        if total_coins >= cls.MILESTONES[-1]:
            current_milestone = cls.MILESTONES[-1]
            next_milestone = cls.MILESTONES[-1]
            progress_pct = 100.0
        else:
            range_span = next_milestone - current_milestone
            earned_in_range = total_coins - current_milestone
            progress_pct = round((earned_in_range / range_span) * 100.0, 1) if range_span > 0 else 100.0

        recent_entries = list(
            StudentCoinLedger.objects.filter(student=user)
            .select_related('attempt', 'attempt__assessment')
        ...
        history = [
            {
                ...
                "assessment_title": entry.attempt.assessment.title if entry.attempt and entry.attempt.assessment else "Assessment",
        ...
        return {
            ...
            "milestone_progress_percentage": progress_pct,
            "milestones": cls.MILESTONES,
            "history": history,
        }

    @classmethod
    def get_leaderboard(cls, current_user: Optional[User] = None, limit: int = 50) -> Dict[str, Any]:
        """
        Deterministic leaderboard ranking:
        ORDER BY coins DESC, name ASC, id ASC.
        Strict privacy safety: does NOT expose emails or internal user IDs to other students.
        """
        from apps.results.models import StudentCoinLedger
        from apps.accounts.models import User, Role
        ...
        students = (
            ...
            .annotate(
                total_coins=Coalesce(Sum('coin_ledger_entries__coins_awarded'), Value(0))
            )
            .order_by('-total_coins', 'display_name', 'id')
        )
        ...
        for rank, st in enumerate(students, start=1):
            ...
            name = (
                ...
                else (st.display_name or f"Student {roll}")
            )
            is_me = (current_user is not None and st.id == current_user.id)
            if is_me:
                user_rank = rank
                user_coins = st.total_coins

            if len(entries) < limit or is_me:
                entries.append({
                    "rank": rank,
                    "name": name,
                    "roll_number": f"{roll[:2]}***{roll[-2:]}" if (len(roll) > 4 and not is_me) else roll,
                    "coins": st.total_coins,
...
class ResultFinalizationService:
    ...
    def finalize_attempt(
        ...
        # Authoritative append-only +3 coin rewards per correct question upon finalized evaluation
        CoinRewardService.award_coins_for_finalized_attempt(
            attempt=attempt,
            question_results=question_results_to_create
        )
