...
from apps.assessments.models import Assessment, AssessmentAssignment, AssignmentStatus, TestAttempt, AttemptStatus
from .models import (
    AssessmentResult,
    QuestionResult,
...
class AdminAssessmentResultListView(APIView):
    ...
    def get(self, request, assessment_id):
        ...
        # Map existing results and attempts by student_id (latest takes precedence)
        results_map = {}
        for res in AssessmentResult.objects.filter(assessment=assessment).select_related(
            'student', 'student__student_profile', 'attempt', 'attempt__proctoring_session'
        ).order_by('created_at'):
            results_map[res.student_id] = res

        attempts_map = {}
        for att in TestAttempt.objects.filter(assessment=assessment).select_related(
        ...
        for asgn in assignments:
            ...
            if stu.id in results_map:
                ...
                elif res.is_released:
                    status_str = 'RELEASED'
                elif res.status in [ResultStatus.PENDING, ResultStatus.PROCESSING]:
                ...
                roster_items.append({
                    ...
                    'proctoring_summary': {
                        'risk_score': str(proc.risk_score),
                        'risk_band': proc.risk_band,
                        'status': proc.status
                    } if proc else None
                })
            elif stu.id in attempts_map:
                att = attempts_map[stu.id]
                proc = getattr(att, 'proctoring_session', None)

                if att.status == AttemptStatus.CANCELLED and proc and proc.status == 'TERMINATED':
                    status_str = 'DISQUALIFIED'
                elif att.status == AttemptStatus.SUBMITTED:
                    status_str = 'EVALUATING'
                ...
                roster_items.append({
                    ...
                    'is_passed': False,
                    'is_released': False,
                    'time_spent_seconds': int((att.submitted_at - att.started_at).total_seconds()) if (att.started_at and att.submitted_at) else (int((timezone.now() - att.started_at).total_seconds()) if att.started_at else 0),
                    'finalized_at': None,
                    'proctoring_summary': {
                        'risk_score': str(proc.risk_score),
                        'risk_band': proc.risk_band,
...
# ==============================================================================
# Participation Certificate Endpoints
# ==============================================================================

class StudentCertificateListView(APIView):
    """
    List all participation certificates earned by the authenticated student.
    """
    permission_classes = [IsAuthenticated, IsActiveUser, IsStudent]

    def get(self, request):
        queryset = Certificate.objects.filter(student=request.user).select_related(
            'exam', 'attempt'
        ).order_by('-created_at')
...
class StudentCertificateDetailView(APIView):
    """
    Retrieve single certificate details for the authenticated student.
    """
    permission_classes = [IsAuthenticated, IsActiveUser, IsStudent]

    def get(self, request, pk):
        cert = get_object_or_404(
            Certificate.objects.select_related('exam', 'student', 'attempt'),
            id=pk
        )
        if cert.student != request.user:
            raise PermissionDenied("You are not authorized to view this certificate.")
...
class StudentCertificateDownloadView(APIView):
    """
    Secure download of participation certificate PDF.
    Enforces server-authoritative ownership: cert.student == request.user.
    """
    permission_classes = [IsAuthenticated, IsActiveUser, IsStudent]
    ...
    def get(self, request, pk):
        ...
        if cert.student != request.user:
            raise PermissionDenied("You are not authorized to download this certificate.")

        if not cert.pdf_file or cert.status != CertificateStatus.ISSUED:
            # On-demand generation fallback
            CertificateService.generate_pdf(cert)
            cert.refresh_from_db()

        if not cert.pdf_file:
            raise Http404("Certificate PDF is not yet available.")
...
class AdminCertificateListView(APIView):
    """
    List, filter, and search certificates across all exams and students.
    """
    permission_classes = [IsAuthenticated, IsActiveUser, IsAdmin]
...
class AdminCertificateDetailView(APIView):
    """
    Admin detail view of a single certificate.
    """
    permission_classes = [IsAuthenticated, IsActiveUser, IsAdmin]
...
class AdminCertificateRetryView(APIView):
    """
    Admin action to retry PDF generation for a failed or pending certificate.
    Preserves all immutable fields (exam, student, attempt, certificate_id, printed_name).
    """
    permission_classes = [IsAuthenticated, IsActiveUser, IsAdmin]

    def post(self, request, pk):
        cert = get_object_or_404(Certificate, id=pk)
        cert = CertificateService.retry_generation(cert, actor=request.user)
        ...
        AuditService.log(
            ...
            target_type="Certificate",
            target_id=str(cert.id),
            metadata={"certificate_id": cert.certificate_id, "status": cert.status},
            request=request
        )

        serializer = CertificateSerializer(cert, context={'request': request})
        return APIResponse(data=serializer.data, message="Certificate generation completed.")


class AdminCertificateDownloadView(APIView):
    """
    Admin download of a certificate PDF.
    """
    permission_classes = [IsAuthenticated, IsActiveUser, IsAdmin]
    ...
    def get(self, request, pk):
        ...
        AuditService.log(
            action="ADMIN_CERTIFICATE_DOWNLOADED",
            actor=request.user,
            target_type="Certificate",
            target_id=str(cert.id),
            metadata={"certificate_id": cert.certificate_id},
...
class PublicCertificateVerifyView(APIView):
    """
    Public verification endpoint for Craft Society certificates.
    No authentication required.
    """
    permission_classes = [AllowAny]

    def get(self, request, certificate_id):
        # Sanitize / safe query: certificate_id can be any string
        clean_id = str(certificate_id).strip()[:100]
        cert = Certificate.objects.filter(certificate_id=clean_id).select_related('exam').first()
        ...
        if not cert:
            return APIResponse(
                data={
                    ...
                    "printed_name": "",
                    "exam_title": "",
                    "organization_name": "Craft Society",
                    "issued_at": None,
                },
        ...
        if cert.status == CertificateStatus.REVOKED:
            return APIResponse(
                data={
                    ...
                    "is_valid": False,
                    "status": "REVOKED",
                    "printed_name": cert.printed_name,
        ...
        if cert.status != CertificateStatus.ISSUED:
            return APIResponse(
                ...
                message=f"Certificate is in {cert.status} state.",
                status_code=status.HTTP_200_OK
            )

        serializer = PublicCertificateVerificationSerializer(cert)
        return APIResponse(data=serializer.data, message="Certificate verified successfully.")


class StudentCoinSummaryView(APIView):
    """
    Retrieve student's total coins, milestone progress, and recent coin transaction ledger.
    GET /api/v1/student/coins/
    """
    permission_classes = [IsAuthenticated, IsActiveUser, IsStudent]

    def get(self, request):
        from .services import CoinRewardService
        summary = CoinRewardService.get_student_coin_summary(request.user)
        return APIResponse(
            data=summary,
            message="Student coin summary retrieved successfully."
        )


class StudentCoinLeaderboardView(APIView):
    """
    Retrieve deterministic student coin leaderboard.
    GET /api/v1/student/coins/leaderboard/
    """
    ...
    def get(self, request):
        ...
        try:
            ...
            limit = 50
        leaderboard_data = CoinRewardService.get_leaderboard(
            current_user=request.user,
            limit=min(100, max(1, limit))
        )
        return APIResponse(
            data=leaderboard_data,
            message="Coin leaderboard retrieved successfully."
        )
