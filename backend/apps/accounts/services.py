...
class AuditService:
    ...
    @classmethod
    def purge_security_audit_trail(
        cls,
        clear_all: bool = False,
        confirm: bool = False,
    ) -> int:
        """
        Safely and permanently clears Security Audit Trail records (AuditLog).
        Requires explicit confirm=True when clear_all=True.
        Executes a bulk QuerySet delete within an atomic transaction.
        Isolated strictly to accounts_auditlog; touches zero other tables.
        Returns the number of deleted records.
        """
        if clear_all and not confirm:
            raise ValueError(
                "Destructive clear-all operation requires explicit confirmation (confirm=True)."
            )
...
class StudentService:
    ...
    """
    @classmethod
    def get_canonical_students_queryset(cls, active_only: bool = True):
        """
        Authoritative queryset for student identity in CODEGUARD.
        Returns StudentProfiles linked to Users with role=STUDENT.
        When active_only=True, excludes deactivated/disabled users.
        Annotates total coins awarded from StudentCoinLedger.
        """
        from django.db.models import Sum, Value
        from django.db.models.functions import Coalesce

        qs = StudentProfile.objects.select_related('user', 'section').filter(
        ...
        qs = qs.annotate(
            coins=Coalesce(
                Sum('user__coin_ledger_entries__coins_awarded'),
                Value(0)
            )
        )
        return qs
    ...
    @classmethod
    def get_canonical_student_users_queryset(cls, active_only: bool = False):
        """
        Authoritative User queryset for eligible student identity in CODEGUARD.
        Returns Users with role=STUDENT who possess a valid StudentProfile.
        Reuses get_canonical_students_queryset to ensure 1:1 parity between
        StudentProfile roster and User eligibility.
        """
        profile_user_ids = cls.get_canonical_students_queryset(active_only=active_only).values_list('user_id', flat=True).distinct()
        return User.objects.filter(id__in=profile_user_ids).select_related('student_profile', 'student_profile__section')
    ...
    def create_student(
        ...
        # Support callers passing roll_number as second arg, or roll_number in email param
        if not roll_number and email and "@" not in email:
            roll_number = email
            email = None

        clean_roll = EUIDService.normalize_roll_number(roll_number)
        clean_email = email.strip().lower() if email and email.strip() else f"{clean_roll.lower()}@student.craftsociety.internal"

        clean_cert = certificate_name.strip()
        clean_disp = clean_cert or (display_name.strip() if display_name else f"{first_name} {last_name}".strip())
        if not clean_disp:
            clean_disp = f"Student {clean_roll}"
        ...
        pwd = initial_password or clean_roll

        try:
            with transaction.atomic():
                # Initial password is the roll number or custom temporary password (hashed securely via set_password)
    ...
    def bulk_create_students(
        ...
        """
        Two-Phase Bulk Student Creation:
        Phase 1: Validate entire batch (in-memory duplicates, database duplicates, valid sections).
                 If ANY error is found, raise DRFValidationError; zero writes occur.
        Phase 2: Atomic bulk creation within transaction.atomic().
                 Database unique constraints act as authoritative concurrency guard.
        """
        if not students_data:
            raise DRFValidationError({"detail": "No student data provided for import."})

        # Phase 1: Pre-validate entire batch
        errors = []
        ...
        for idx, item in enumerate(students_data, start=1):
            raw_roll = (item.get('roll_number') or '').strip()
            raw_email = (item.get('email') or '').strip().lower()
            cert_name = (item.get('certificate_name') or item.get('name') or '').strip()
            sec_code = (item.get('section') or '').strip()
            ...
            if not raw_roll:
                ...
                try:
                    ...
                    if norm_roll in seen_rolls:
                        row_errors.append(f"Duplicate roll number in batch (first seen at row {seen_rolls[norm_roll]}).")
                    else:
                        seen_rolls[norm_roll] = idx
                        candidate_rolls.add(norm_roll)
                except DRFValidationError as ve:
                    row_errors.append(str(ve.detail[0] if isinstance(ve.detail, list) else ve.detail))

            # Email (synthesize if absent)
            clean_email = raw_email
            if not clean_email and norm_roll:
                clean_email = f"{norm_roll.lower()}@student.craftsociety.internal"

            if clean_email:
                try:
                    validate_email(clean_email)
                    if clean_email in seen_emails:
                        row_errors.append(f"Duplicate email in batch (first seen at row {seen_emails[clean_email]}).")
                    else:
                        seen_emails[clean_email] = idx
                        candidate_emails.add(clean_email)
                except DjangoValidationError:
                    row_errors.append("Invalid email address format.")

            normalized_items.append({
                "row_number": idx,
                "roll_number": norm_roll or raw_roll,
                "email": clean_email,
        ...
        # Check DB uniqueness for candidates
        ...
        for norm_item in normalized_items:
            row_errors = norm_item["errors"]
            roll = norm_item["roll_number"]
            email = norm_item["email"]
            sec = norm_item["section"]

            if roll in existing_rolls:
                row_errors.append(f"Roll number '{roll}' already exists in system database.")
            if email in existing_emails:
                row_errors.append(f"Email '{email}' already exists in system database.")

            if row_errors:
                errors.append({
                    "row": norm_item["row_number"],
                    "roll_number": roll,
                    "email": email,
                    "errors": row_errors
                })

        # If ANY error exists across the entire dataset, fail fast with ZERO database writes
        if errors:
            raise DRFValidationError({
                "detail": f"Import rejected: {len(errors)} row(s) failed validation. Zero records written.",
                "errors": errors
            })
        ...
        try:
            with transaction.atomic():
                for norm_item in normalized_items:
                    user, profile = cls.create_student(
                        email=norm_item["email"],
                        roll_number=norm_item["roll_number"],
                        first_name=norm_item["first_name"],
                        last_name=norm_item["last_name"],
                        certificate_name=norm_item.get("certificate_name", ""),
                        section=None,
                        actor=actor,
                        request=request
                    )
                    created_records.append((user, profile))
        except (IntegrityError, DRFValidationError) as e:
            # Concurrency collision against UniqueConstraint or validation
            raise DRFValidationError({
                "detail": f"Import aborted due to concurrent record creation conflict: {str(e)}"
            })
...
class ImportService:
    ...
    def parse_file(cls, uploaded_file) -> List[Dict[str, str]]:
        ...
        if filename.endswith('.csv'):
            try:
                ...
                roll_idx, email_idx, name_idx, section_idx = cls._detect_columns(header_row)

                for line_num, row in enumerate(reader, start=2):
                    if not any(cell.strip() for cell in row):
                        continue  # Skip blank lines
                    roll = row[roll_idx].strip() if len(row) > roll_idx and roll_idx != -1 else ""
                    email = row[email_idx].strip() if len(row) > email_idx and email_idx != -1 else ""
                    name = row[name_idx].strip() if len(row) > name_idx and name_idx != -1 else ""
            ...
            try:
                ...
                roll_idx, email_idx, name_idx, section_idx = cls._detect_columns(header_list)

                for line_num, row in enumerate(iter_rows, start=2):
                    if not row or not any(str(cell).strip() for cell in row if cell is not None):
                        continue
                    roll = str(row[roll_idx]).strip() if len(row) > roll_idx and roll_idx != -1 and row[roll_idx] is not None else ""
                    email = str(row[email_idx]).strip() if len(row) > email_idx and email_idx != -1 and row[email_idx] is not None else ""
                    name = str(row[name_idx]).strip() if len(row) > name_idx and name_idx != -1 and row[name_idx] is not None else ""
                    has_sec = (section_idx != -1)
    ...
    @staticmethod
    def _detect_columns(header_row: List[str]) -> Tuple[int, int, int, int]:
        roll_aliases = ['roll number', 'roll_number', 'roll', 'rollno', 'roll_no', 'student roll number', 'student roll', 'id']
        email_aliases = ['email', 'email address', 'email_address', 'student email', 'mail']
        name_aliases = ['name', 'student name', 'student_name', 'full name', 'full_name', 'certificate name', 'certificate_name']
        ...
        for idx, col in enumerate(header_row):
            ...
            if col_clean in roll_aliases and roll_idx == -1:
                ...
                email_idx = idx
            elif col_clean in name_aliases and name_idx == -1:
                name_idx = idx
        ...
        if roll_idx == -1:
            raise DRFValidationError(
                "Required column header 'Roll Number' not found in file."
            )

        return roll_idx, email_idx, name_idx, section_idx

    @classmethod
    def validate_preview(cls, rows: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        Validates parsed rows independently, checks database uniqueness,
        ...
        for r in rows:
            ...
            if roll:
                ...
                if not email:
                    candidate_emails.add(f"{roll.lower()}@student.craftsociety.internal")
            if email:
        ...
        for r in rows:
            ...
            if raw_email:
                ...
                if clean_email in existing_emails:
                    errors.append(f"Generated email '{clean_email}' already exists in system database.")
                    status_tag = "DUPLICATE"
            ...
            preview_rows.append({
                ...
                "email": clean_email or raw_email,
                "name": raw_name,
                "section": raw_sec,
                "euid": euid_preview,
                "status": status_tag,
    ...
    def execute_import(
        ...
        """
        Executes student creation for all valid rows with per-row atomic isolation.
        Populates created_students, temporary credentials list, and failed_rows.
        """
        created_students = []
        credentials_list = []
        failed_rows = []

        all_sections = {s.code.upper(): s for s in Section.objects.all()}

        for item in items:
            raw_roll = (item.get('roll_number') or '').strip()
            raw_email = (item.get('email') or '').strip().lower()
            name = (item.get('name') or item.get('certificate_name') or '').strip()
            first_name = (item.get('first_name') or '').strip()
            last_name = (item.get('last_name') or '').strip()
            sec_code = (item.get('section') or '').strip().upper()
            sec_obj = all_sections.get(sec_code) if sec_code else None

            # Initial temporary password is the student's roll number
            temp_password = raw_roll
            ...
            try:
                with transaction.atomic():
                    ...
                    credentials_list.append({
                        ...
                        "name": profile.certificate_name or user.display_name
                    })
            except (DRFValidationError, DjangoValidationError, IntegrityError, Exception) as exc:
                err_msg = str(exc)
                if isinstance(exc, DRFValidationError) and isinstance(exc.detail, dict):
                    err_msg = "; ".join(f"{k}: {v}" for k, v in exc.detail.items())
                elif isinstance(exc, DRFValidationError) and isinstance(exc.detail, list):
                    err_msg = "; ".join(str(v) for v in exc.detail)
                failed_rows.append({
                    **item,
                    "error": err_msg
                })
...
class AccountSecurityService:
    ...
    def set_administrator_status(
        ...
        """
        Enables or disables a secondary administrator account.
        ONLY the Primary Administrator may activate or deactivate an administrator account.
        Rejects self-deactivation, primary admin deactivation, and deactivation of the last active administrator.
        """
        if target_admin.role != Role.ADMIN:
            raise DRFValidationError({"detail": "Target user is not an administrator."})

        if target_admin.is_primary_admin:
            raise DRFValidationError({"detail": "Primary Administrator account cannot be disabled."})

        if actor and not getattr(actor, 'is_primary_admin', False):
            raise PermissionDenied("Only the Primary Administrator can modify administrator status.")

        if actor and str(actor.id) == str(target_admin.id):
            raise DRFValidationError({"detail": "Cannot deactivate your own administrator account."})

        if not is_active:
            active_count = User.objects.filter(role=Role.ADMIN, is_active=True).exclude(id=target_admin.id).count()
            if active_count == 0:
                raise DRFValidationError({"detail": "At least one active administrator must remain in the system."})

        if target_admin.is_active != is_active:
            ...
            action = "ADMIN_ENABLED" if is_active else "ADMIN_DISABLED"
            AuditService.log(
                action=action,
                actor=actor,
                target_type="User",
                ...
                metadata={
                    ...
                    "reason": reason.strip() if reason else ("Administrator enabled." if is_active else "Administrator disabled."),
                    "result": "SUCCESS"
                },
                request=request
            )
    ...
    def delete_administrator(
        ...
        Only Primary Administrator may delete administrators.
        Primary Admin and self cannot be deleted.
        Checks active duties, active legal holds, and retention policies.
        Re-attributes created assessments and assignments to Primary Admin to maintain referential integrity.
        Revokes sessions, logs immutable audit snapshot, and deletes user.
        """
        if target_admin.role != Role.ADMIN:
            raise DRFValidationError({"detail": "Target user is not an administrator."})
        ...
        if actor and not getattr(actor, 'is_primary_admin', False):
            raise PermissionDenied("Only the Primary Administrator can delete administrator accounts.")
        ...
        from apps.assessments.models import Assessment, AssessmentAssignment
        ...
        primary_admin = actor if (actor and getattr(actor, 'is_primary_admin', False)) else User.objects.filter(primary_admin_marker='PRIMARY').first()
        ...
        with transaction.atomic():
            if primary_admin:
                Assessment.objects.filter(created_by=target_admin).update(created_by=primary_admin)
                AssessmentAssignment.objects.filter(assigned_by=target_admin).update(assigned_by=primary_admin)
            target_admin.delete()

        return {
            "status": "deleted",
            "message": f"Administrator account {target_admin.email} ({target_admin.admin_id}) deleted successfully."
        }
    ...
    def bulk_delete_administrators(
        ...
        for aid in admin_ids:
            ...
            try:
                res = cls.delete_administrator(target_admin=target_admin, actor=actor, request=request)
                status_action = res.get("status", "deleted") if isinstance(res, dict) else "deleted"
                results.append({
                    "id": str(aid),
                    "email": email,
                    "admin_id": admin_id,
