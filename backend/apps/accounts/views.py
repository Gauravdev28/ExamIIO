...
class ChangePasswordView(APIView):
    ...
    def post(self, request):
        ...
        update_fields = ['password', 'first_login_required', 'updated_at']

        cert_name = serializer.validated_data.get('certificate_name', '').strip()
        first_name = serializer.validated_data.get('first_name', '').strip()
        last_name = serializer.validated_data.get('last_name', '').strip()
        effective_name = cert_name or f"{first_name} {last_name}".strip()

        if effective_name:
            user.display_name = effective_name
            update_fields.append('display_name')

        user.save(update_fields=update_fields)

        # If student, clear first_login_required and update certificate_name on student profile
        if user.role == Role.STUDENT and hasattr(user, 'student_profile'):
            profile = user.student_profile
            profile_update_fields = ['first_login_required', 'updated_at']
            profile.first_login_required = False
            if cert_name:
                profile.certificate_name = cert_name
                profile_update_fields.append('certificate_name')
            elif effective_name and not profile.certificate_name:
                profile.certificate_name = effective_name
                profile_update_fields.append('certificate_name')
...
class AdminStudentListView(APIView):
    ...
    def get(self, request):
        ...
        if status_filter is not None:
            ...
            queryset = StudentService.get_canonical_students_queryset(active_only=is_active)
        else:
            queryset = StudentService.get_canonical_students_queryset(active_only=False)
...
class AdminDashboardOverviewView(APIView):
    ...
    def get(self, request):
        ...
        total_students = StudentService.get_canonical_students_queryset(active_only=True).count()
...
class AdministratorDetailView(APIView):
    ...
    def delete(self, request, pk):
        ...
        res = AccountSecurityService.delete_administrator(target_admin=target_admin, actor=request.user, request=request)

        msg = res.get("message") if isinstance(res, dict) and res.get("message") else f"Administrator account {admin_email} ({admin_id}) deleted successfully."
        return APIResponse(
            message=msg,
...
class AdministratorStatusView(APIView):
    ...
    Enables/disables an administrator account. Rejects non-primary admin callers, self-deactivation,
    primary admin deactivation, and deactivation of the last active administrator.
    """
    permission_classes = [IsAuthenticated, IsAdmin, IsActiveUser]
    ...
    def post(self, request, pk):
        ...
        if not getattr(request.user, 'is_primary_admin', False):
            return APIResponse(
                status_code=status.HTTP_403_FORBIDDEN,
                message="Only the Primary Administrator can modify administrator status.",
                error={"code": "FORBIDDEN", "message": "Permission denied."}
            )
        ...
        if desired_status is None:
            desired_status = not target_admin.is_active
        else:
            desired_status = bool(desired_status)

        reason = request.data.get('reason', '').strip()

        try:
            target_admin = AccountSecurityService.set_administrator_status(
            ...
            if hasattr(e, 'detail'):
                err_detail = e.detail.get('detail') if isinstance(e.detail, dict) else str(e.detail)
            elif hasattr(e, 'message_dict'):
                err_detail = str(e.message_dict)
            elif hasattr(e, 'messages'):
                err_detail = e.messages[0] if e.messages else str(e)
            else:
                err_detail = str(e)
            code = "LAST_ADMIN_PROTECTED" if "at least one active" in str(err_detail).lower() else "ADMIN_STATUS_ERROR"
            return APIResponse(
                message=err_detail or "Failed to update administrator status.",
                status_code=status.HTTP_400_BAD_REQUEST,
                error={"code": code, "message": err_detail}
            )
            ...
            return APIResponse(
                status_code=status.HTTP_403_FORBIDDEN,
                message=msg or "Only the Primary Administrator can modify administrator status.",
