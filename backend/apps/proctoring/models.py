...
class ProctoringEvent(UUIDModel):
    ...
    is_confirmed = models.BooleanField(
        default=True,
        db_index=True,
        help_text="True if event is an authoritative confirmed disciplinary violation, False for raw detection hints."
    )
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(fields=['session', 'is_confirmed'], name='idx_event_session_confirmed'),
        ]
...
class IntegrityIncident(UUIDModel, TimeStampedModel):
    """
    Idempotent ledger for client-detected integrity incidents (tab switches, focus loss).
    Guarantees that repeated/retried incident reports with the same client_incident_id are deduplicated.
    """
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=['attempt', 'client_incident_id'],
                name='unique_attempt_client_incident'
            )
        ]
        indexes = [
            models.Index(fields=['attempt', 'client_incident_id'], name='idx_incid_att_client_id'),
        ]

    def __str__(self):
        return f"IntegrityIncident {self.client_incident_id} on Attempt {self.attempt_id} ({self.incident_type})"
